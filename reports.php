...
if (isset($_GET['delete_report']) && $_SESSION["user_status"] === 'admin') {
...
<body class="df body">
    ...
    <main class="main">
        ...
        <div class="table-content">
            <?php
            $sql = "SELECT R.Report_ID, R.Report_Name, R.Report_Description, R.Report_Date, E.First_Name, E.Last_Name 
            FROM Reports R
            ...
            <?php } ?>
                    ...
                    ?>
                        <tr>
                            ...
                            <td><a href="report.php?report_id=<?php echo $row["Report_ID"] ?>"><?php echo $row["Report_Name"] ?></a></td>
                            <td><?php echo $row["First_Name"] ?></td>
                            <td><?php echo $row["Last_Name"] ?></td>

                            <td><?php echo date('Y-m-d', strtotime($row["Report_Date"])) ?></td>
                            <!-- <td title="<?php echo $row["Report_Description"] ?>"><?php echo $row["Report_Description"] ?></td> -->


                            <?php if ($_SESSION["user_status"] === 'admin') { ?>
                            <td><a href="edit_report.php?edit_report=<?php echo $row["Report_ID"] ?>">Изменить</a></td>
                            <td><a href="reports.php?delete_report=<?php echo $row["Report_ID"] ?>" onclick='return confirm("Вы уверены, что хотите удалить этот отчет?")'>Удалить</a></td>
                            <?php } ?>
                ...
                <?php
                    ...
                    echo "<div class='warning'>Нет отчетов</div>";
                }
                $conn->close();
                ?>
        </div>
        ...
        <div class="table-btns btns">
                <a class="btn" href="add_reports.php">Добавить отчет</a>
        </div>
